...
def process_file(file_path):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            if "barve" in line:
                # Extract the color part and check for 'simple' keyword
                parts = line.split(';')
                colors_part = parts[1].split('!')[1]
                colors = colors_part.split(',')
                num_squares=parts[1].split('!')[0]
                print(num_squares)
                print(colors_part)
                print(parts)
                print(colors)
                if 'simple' in parts[0]:
                    ...
                    print("The line does not have the 'simple' keyword.")
                break

# Run the script with the file path
process_file('/media/lmk/stopnice/izvedba.txt')
